...
// NOTE: Adjusted version to work with export object module loading.
...
ko.bindingHandlers.module = {
            ...
            read: function() {
                ...
                if (moduleName) {
                    moduleLoader(moduleName, function(mod) {
                        ...
                        // BEGIN: Change: Handle multi export modules
                        if (ko.bindingHandlers.module.multiExportHandling && typeof mod === "object") {

                            // if the multi export object contains a default property: use that one
                            if ("default" in mod) {
                                mod = mod.default;
                            }
                            else {

                                // if the module contains only one export (that is wrapped): use that one
                                var exportKeys = Object.keys(mod);
                                if (exportKeys.length === 1) {
                                    mod = mod[exportKeys[0]];
                                }
                            }
                        }
    ...
    // BEGIN: Change: new flag to change behavior
    multiExportHandling: false
    // END: Change
};
